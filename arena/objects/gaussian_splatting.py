...
class GaussianSplatting(Object):
    """
    Class for GaussianSplatting in the ARENA: Load a 3D Gaussian Splat for Real-Time Radiance Field Rendering.  More information : (https://github.com/quadjr/aframe-gaussian-splatting) A-Frame Gaussian Splatting. See guidance to store paths under (https://docs.arenaxr.org/content/interface/filestore.html) ARENA File Store, CDN, or DropBox.
    
    :param str src: Store File Store file paths under 'store/users/<username>', see CDN and other storage options in the description above. (optional)
