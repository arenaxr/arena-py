...
def on_msg_callback(scene, evt, _msg):
    if isinstance(evt, Event) and evt.type == "mousedown":
        print(f"Program 2 mousedown event: {evt.data.target}")
        obj = scene.get_persisted_obj(evt.data.target)
