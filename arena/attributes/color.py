    ...
    def __init__(self, red=0, green=0, blue=0):
        if isinstance(red, str):
            ...
            if re.match(r"^[0-9a-fA-F]{3}$", color):
                # 3-digit hex
                c = tuple(int(color[i] * 2, 16) for i in range(3))
            elif re.match(r"^[0-9a-fA-F]{6}$", color):
                # 6-digit hex
                c = tuple(int(color[i:i + 2], 16) for i in (0, 2, 4))
            else:
                # Try as web color name
                wcrgb = name_to_rgb(color)
...
# Replacement for webcolors.name_to_rgb()
COLOR_NAMES = {
...
def name_to_rgb(color_name):
    ...
    Returns RGB tuple or None if color not found.
    """
    color_name = color_name.lower().strip()
    return COLOR_NAMES.get(color_name)