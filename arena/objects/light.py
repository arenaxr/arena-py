...
class Light(Object):
    ...
    :param bool castShadow: Whether this light casts shadows on the scene. Defaults to 'False' (optional)
    :param str color: Light color. For 'hemisphere', light color from above. Defaults to '#ffffff' (optional)
    :param float decay: Amount the light dims along the distance of the light. Requires `type: point` or `spot`. Defaults to '1' (optional)
    :param float distance: Distance where intensity becomes 0. If distance is 0, then the point light does not decay with distance. Requires `type: point` or `spot`. Defaults to '0' (optional)
    ...
    :param bool shadowCameraVisible: Displays a visual aid showing the shadow camera's position and frustum. This is the light's view of the scene, used to project shadows. Defaults to 'False' (optional)
    ...
    :param str target: Id of element the spot should point to. Set to null to transform spotlight by orientation, pointing to it's -Z axis. Requires `type: spot`. Defaults to 'None' (optional)
