...
class Animation(Attribute):
    ...
    :param float delay: How long (milliseconds) to wait before starting. Defaults to '0' (optional)
    ...
    :param str from: Initial value at start of animation. If not specified, the current property value of the entity will be used (will be sampled on each animation start). It is best to specify a from value when possible for stability. Defaults to '' (optional)
    :param bool isRawProperty: Flag to animate an arbitrary object property outside of A-Frame components for better performance. If set to true, for example, we can set property to like components.material.material.opacity. If property starts with components or object3D, this will be inferred to true. Defaults to 'False' (optional)
    :param str loop: How many times the animation should repeat. If the value is true, the animation will repeat infinitely. Defaults to '0' (optional)
    :param list[str] pauseEvents: Comma-separated list of events to listen to trigger pause. Can be resumed with resumeEvents. Defaults to '[]' (optional)
    ...
    :param list[str] resumeEvents: Comma-separated list of events to listen to trigger resume after pausing. Defaults to '[]' (optional)
    :param bool round: Whether to round values. Defaults to 'False' (optional)
    :param list[str] startEvents: Comma-separated list of events to listen to trigger a restart and play. Animation will not autoplay if specified. startEvents will restart the animation, use pauseEvents to resume it. If there are other animation components on the entity animating the same property, those animations will be automatically paused to not conflict. Defaults to '[]' (optional)
    :param str to: Target value at end of animation. Defaults to '' (optional)
    :param str type: Right now only supports color for tweening isRawProperty color XYZ/RGB vector values. Defaults to '' (optional)
    """
