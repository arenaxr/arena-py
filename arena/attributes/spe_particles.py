...
class SpeParticles(Attribute):
    ...
    :param list[dict] colorSpread: Spread to apply to colors, spread an array of vec3 (r g b) with 0 for no spread. note the spread will be re-applied through-out the lifetime of the particle. Defaults to '[]' (optional)
    ...
    :param bool depthWrite: If true, particles write their depth into the depth buffer. this should be false if we use transparent particles. Defaults to 'False' (optional)
    ...
    :param bool enableInEditor: Enable the emitter while the editor is active (i.e. while scene is paused). Defaults to 'False' (optional)
    :param bool enabled: Enable/disable the emitter. Defaults to 'True' (optional)
    :param bool frustumCulled: Enable/disable frustum culling. Defaults to 'False' (optional)
    ...
    :param bool randomizeAcceleration: If true, re-randomize acceleration when re-spawning a particle, can incur a performance hit. Defaults to 'False' (optional)
    :param bool randomizeAngle: If true, re-randomize angle when re-spawning a particle, can incur a performance hit. Defaults to 'False' (optional)
    :param bool randomizeColor: If true, re-randomize colour when re-spawning a particle, can incur a performance hit. Defaults to 'False' (optional)
    :param bool randomizeDrag: If true, re-randomize drag when re-spawning a particle, can incur a performance hit. Defaults to 'False' (optional)
    :param bool randomizeOpacity: If true, re-randomize opacity when re-spawning a particle, can incur a performance hit. Defaults to 'False' (optional)
    :param bool randomizePosition: If true, re-randomize position when re-spawning a particle, can incur a performance hit. Defaults to 'False' (optional)
    :param bool randomizeRotation: If true, re-randomize rotation when re-spawning a particle, can incur a performance hit. Defaults to 'False' (optional)
    :param bool randomizeSize: If true, re-randomize size when re-spawning a particle, can incur a performance hit. Defaults to 'False' (optional)
    :param bool randomizeVelocity: If true, re-randomize velocity when re-spawning a particle, can incur a performance hit. Defaults to 'False' (optional)
    ...
    :param bool rotationStatic: If true, the particles are fixed at their initial rotation value. if false, the particle will rotate from 0 to the 'rotation' value. Defaults to 'False' (optional)
    ...
    :param str texture: Texture to be used for each particle, may be a spritesheet. Examples: [blob.png, fog.png, square.png, explosion_sheet.png, fireworks_sheet.png], like path 'static/images/textures/blob.png'. Defaults to '' (optional)
